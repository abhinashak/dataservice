...
import com.bizosys.dataservice.dao.IPool;
...
public class CachedIdGenerator {
	...
	private final static Logger LOG = Logger.getLogger(CachedIdGenerator.class);
	...
	public static final class IdCounter {
		...
		public int bufferCounter = DataserviceConstants.DEFAULT_VALUE;
	...
	public int generateId( String sequenceKey ) throws SQLException {
		...
		synchronized (sequenceKey) {
			...
			if( null == idcounter || idcounter.bufferCounter == 0 ) {
				...
				int currentId = 0;
				Integer cacheAmount = DataserviceConstants.CACHED_AMOUNT.get(sequenceKey);
				if( null == cacheAmount ) cacheAmount = DataserviceConstants.DEFAULT_VALUE;
