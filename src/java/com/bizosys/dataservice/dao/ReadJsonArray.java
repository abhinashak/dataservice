...
public class ReadJsonArray extends ReadBase<String> {
	...
	private final static Logger LOG = Logger.getLogger(ReadJsonArray.class);
	...
	protected List<String> populate() throws SQLException {
		...
		while (this.rs.next()) {
			...
			if( DEBUG_ENABLED ) {
				if(count++ % DataserviceConstants.DB_RCORDS_FETCH_LOG_INTERVAL == 0)
	...
	public String getColObjStr(Object colObj, int type)
	{
		...
		switch (type) {
			...
			default:
			{
				...
				return sb.append("\"").append(JsonStringEncoder.getInstance().quoteAsString(colObj.toString())).append("\"").toString();
			}
		}
	}
