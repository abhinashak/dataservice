...
public class PoolFactory 
{
			...
			if (dbcL != null && !dbcL.isEmpty())
			{
				for (DbConfig config : dbcL)
				{
					...
					if ( config.defaultPool) 
						DbConfig.DEFAULT_PREPARE_STMT_SUPPORT = config.preparedStmt;
				}
				return true;
			}
		}
	...
	private synchronized void startPool(DbConfig config)
	{
		...
		if (this.poolMap.containsKey(config.poolName) ) {
			LOG.info("Pool is already started Ignoring - " + config.poolName);
			return;
		}

		IPool pool = new Pool(config.poolName);
		this.poolMap.put(config.poolName, pool);
		pool.start(config);
