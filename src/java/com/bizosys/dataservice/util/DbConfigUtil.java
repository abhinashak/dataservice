...
import com.bizosys.dataservice.dao.DbConfig;
import com.bizosys.dataservice.dao.PoolFactory;
import com.bizosys.dataservice.dao.ReadArray;

public class DbConfigUtil {
	
	private static ILogger LOG = LoggerFactory.getLogger(DbConfigUtil.class, DataserviceConstants.IS_CONSOLE_LOG);
	...
	private static final String[] setPools(final String query, final Object[] params) throws SQLException {
		...
		reader.setPoolName(DataserviceConstants.CONFIG_POOL);
	...
	public static final void initializePool(Object[] poolUrlUserPass) {
		...
		config.connectionUrl = poolUrlUserPass[1].toString() + DataserviceConstants.CONNECTION_URL_OPTION;
		config.login = poolUrlUserPass[2].toString();  
		config.password = poolUrlUserPass[3].toString();
		config.driverClass = DataserviceConstants.JDBC_DRIVER;
